...
actor AVAudioEnginePipeline: AudioPipeline {
    ...
    static let silenceSamplesThreshold = 12800 // 800 ms at 16 kHz
    static let minimumSpeechSamples = 1600 // 100 ms at 16 kHz
    static let partialSnapshotSamplesThreshold = 3200 // 200 ms at 16 kHz
    ...
    private func emitCurrentSegment(endedBySilence: Bool) {
        guard inSpeech, speechSampleCount >= Self.minimumSpeechSamples else {
            return
        }
    ...
    private func emitSnapshotIfNeeded() {
        guard inSpeech,
              ...
              samplesSinceLastSnapshot >= Self.partialSnapshotSamplesThreshold
        else {
            return
        }
